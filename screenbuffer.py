class ScreenBuffer(object):
    ...
    def __init__(self, height, width):
        ...
        self.height = height
        self.width = width
        self.lines = [[(' ', None, None) for y in range(width)] for i in range(height)]
        self.prev_modes = (None, None)
    ...
    def debug(self):
        ...
        for i,l in enumerate(self.lines):
            m = self.modes[i]
            s = [(l[x], m[x]) for x in range(len(m))]
            p.append('%d: %s' % (i, s))
        return '\n'.join(p)
    ...
    def diff(self, old):
        ...
        for y in range(0, self.height):
            for x in range(0, self.width):
                oc, of, ob = old.lines[y][x]
                nc, nf, nb = self.lines[y][x]
                if (oc, of, ob) != (nc, nf, nb):
                    changed_coords.append((x, y))
        return changed_coords

    def compile_char(self, x, y, res):
        c, f, b = self.lines[y][x]
        of, ob = self.prev_modes
        if f != of:
    ...
    def compile(self, old=None):
        ...
        # Compile diff
        changed_coords = self.diff(old)
        self.prev_modes = old.prev_modes
